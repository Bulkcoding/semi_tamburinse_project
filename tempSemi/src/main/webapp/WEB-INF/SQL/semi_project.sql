...
--	// ** U구매자이름, IM상품사진, IT상품이름, R리뷰작성날짜, R작성리뷰 ** //
-- 상품 이름 가져오기

SELECT re_date, re_content, it_name, img_file, name, userid
FROM
(
    ...
    (
        ...
        from tbl_item
    --    where fk_ca_id = 'bw'
    )IT ON IT.it_seq_no = D.fk_it_seq_no JOIN
    (
        select fk_it_seq_no, img_file
        from tbl_img
        where is_main_img = 1
...
select seq, fk_userid, name, subject
     , readCount, to_char(regDate, 'yyyy-mm-dd hh24:mi:ss')
from tbl_board
where status = 1
order by seq desc
...
select lag(seq, 1) over(order by seq desc) AS previousseq
     -- seq(글번호)의 내림차순으로 정렬했을때 위쪽으로 1칸 올라간 행에서 boardno 컬럼의 값을 가져온다.  
     , lag(subject, 1) over(order by seq desc) AS previoussubject
    -- seq(글번호)의 내림차순으로 정렬했을때 위쪽으로 1칸 올라간 행에서 subject 컬럼의 값을 가져온다.   
...
-- 오라클 계정명은 semi_orauser2 이고 암호는 gclass 인 사용자 계정을 생성합니다.
